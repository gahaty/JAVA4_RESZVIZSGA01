...
public class CommentRepository {
	...
	public List<Comment> lookFor(Integer id, String lookFor) {
		String query = "SELECT comment.id, user.user_name, comment.comment, comment.datetime FROM comment INNER JOIN user ON user.id = user_name_id WHERE post_id = ? AND (comment Like '%"
				+ lookFor + "%' OR user.user_name LIKE '%" + lookFor + "%')";
		return template.query(query, new BeanPropertyRowMapper<>(Comment.class), id);
	}
}
